...
int main() {
    ...
    const auto mic = make_packet(0, 48000, 1, 0.25f, 1);
    const auto system = make_packet(0, 48000, 2, 0.50f, 2);

    if (!require(mixer.push("microphone", mic), "microphone packet accepted")) return 1;
    if (!require(mixer.push("system", system), "system packet accepted")) return 1;
